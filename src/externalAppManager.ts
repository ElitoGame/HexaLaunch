...
export class externalAppManager {
	...
	public static async getSearchDatabase() {
		if (this.appDB === undefined) {
			...
			if (this.appData.length === 0) {
				...
				this.queryOtherApps();
				// fs.readTextFile("appData.json", { dir: BaseDirectory.AppData }).then(
				// 	(data) => {
				// 		const parsed = JSON.parse(data);
				// 		const nameArray = this.getApplicationNameFromExe(parsed);
				// 		console.log("Read appData.json", nameArray);
				// 	}
				// );
			}
			await insertBatch(
	...
	public static async queryRelevantApps(): Promise<externalApp[]> {
		...
		customApps.forEach(
			(x: { name: string; executable: string; icon: string }) => {
		...
		// unique
		appData.filter(
			(x, i, arr) => arr.findIndex((y) => y.executable === x.executable) === i
		);
	...
	public static async queryOtherApps(): Promise<externalApp[]> {
		...
		const command = new Command("queryRelevantApps", [
			JSON.parse(JSON.stringify(queryOtherApps)),
		]);
		command.on("close", (data) => {
			console.log(
				`command finished with code ${data.code} and signal ${data.signal} in ${
		...
		const nameArray = await this.getApplicationNameFromExe(
			appData.map((x) => x.executable)
		);
		...
		this.appData = this.appData.concat(
			appData.filter(
				(x) => !this.appData.find((y) => y.executable === x.executable)
			)
		);
		...
		console.log(
			"Done querying other apps: " + this.appData.length,
			this.appData
		);
		return appData;
	}
	...
	public static async addCustomApp(executable: string) {
		...
		if (
			this.appDataCustomApps.find((x) => x.executable === executable) ===
			undefined
		) {
...
░░░░░ ░░░░ ░░░░░  ░░░░░░░░ ░░░░░    ░░░░░    ░░░░░  ░░░░░░░░   ░░░░░░░░  ░░░░░░░░ ░░░░░     ░░░░░███   ░░░░░░░░  ░░░░░░  ░░░░░     ░░░░░  ░░░░░░  ░░░░░░  
  ...
  */
	...
	private static async queryEpicGames() {
		...
		if (!epicPath) return [];
		let collectedApps: Array<externalApp> = await new Promise(
			async (resolve) => {
	...
	private static async querySteamGames() {
		...
		const libraryfolders = Object.values(
			this.acf_to_json(
				(
					await fs.readTextFile(`${steamPath}\\steamapps\\libraryfolders.vdf`)
		...
		const collectedSteamGames: Array<externalApp> = await new Promise(
			async (resolve) => {
