      ...
      <Box w="100%" pt="10px" pb="50px" class="flex justify-between">
        ...
        <For each={themes.themes}>
          ...
          {(themeVar: Themes, ) => (
            ...
            <Show when={themeVar.getThemeName() == "Honey" || themeVar.getThemeName() == "Light" || themeVar.getThemeName() == "Dark"}>

              <div class=" roundend-lg card  border">
                ...
                />   <label class="pointer-events-none" for="card1">
                  <Show when={themeVar.getThemeName() == "Honey"}>
                    <svg style={` color:${themeVar.getMainHexagonIcon()}; font-size:30px;`} class="z-20 absolute top-[40px] left-[66px]">
                      <path d="M9.41992 7.69002C9.41992 4.3965 12.0899 1.72656 15.3834 1.72656C18.6769 1.72656 21.3468 4.3965 21.3468 7.69002H9.41992Z" fill="#F68002" />
                      <path d="M21.3472 16.4782C21.3472 19.7717 18.6772 22.4417 15.3837 22.4417C12.0902 22.4417 9.42024 19.7717 9.42024 16.4782L9.42024 14.2811L21.3472 14.2811V16.4782Z" fill="#F68002" />
                      <rect x="9.41992" y="9.57324" width="11.9269" height="2.8248" fill="#F68002" />
                      <path d="M0.569258 15.1998C0.230613 13.6335 0.917017 12.024 2.2818 11.1841L6.5047 8.58539C7.2307 8.13862 8.1655 8.66049 8.1662 9.51294L8.17012 14.3215C8.17177 16.3477 6.56998 18.0307 4.54847 18.1685C2.67707 18.2961 0.965645 17.0332 0.569258 15.1998Z" fill="#F68002" />
                      <path d="M30.316 15.2064C30.6546 13.6401 29.9682 12.0306 28.6035 11.1907L24.3806 8.59198C23.6546 8.14521 22.7198 8.66708 22.7191 9.51954L22.7151 14.3281C22.7135 16.3543 24.3153 18.0373 26.3368 18.1751C28.2082 18.3027 29.9196 17.0398 30.316 15.2064Z" fill="#F68002" />
                      <path fill-rule="evenodd" clip-rule="evenodd" d="M11.6627 0.0474556C12.07 -0.100642 12.5202 0.109457 12.6683 0.516725L13.3875 2.49441C13.5356 2.90168 13.3255 3.35189 12.9182 3.49999C12.5109 3.64808 12.0607 3.43798 11.9126 3.03072L11.1935 1.05303C11.0454 0.645766 11.2555 0.195553 11.6627 0.0474556ZM19.4149 0.0474556C19.8222 0.195553 20.0323 0.645766 19.8842 1.05303L19.165 3.03072C19.0169 3.43798 18.5667 3.64808 18.1594 3.49999C17.7522 3.35189 17.5421 2.90168 17.6902 2.49441L18.4093 0.516725C18.5574 0.109457 19.0076 -0.100642 19.4149 0.0474556Z" fill="#F68002" />
                    </svg>
                  </Show>
                  <Show when={themeVar.getThemeName() == "Light"}>
                    <FaSolidSun color={`${themeVar.getMainHexagonIcon()}`} size={21} class="z-20 absolute top-[40px] left-[72px]" /></Show>
                  <Show when={themeVar.getThemeName() == "Dark"}>
                    <FaSolidMoon color={`${themeVar.getMainHexagonIcon()}`} size={21} class="z-20 absolute top-[40px] left-[72px]" />

                  </Show>
                  <span class="colorPreview">
                    <div class="position-absolute w-5 h-5 m-1 rounded-sm" style={`background-color:${themeVar.getMainHexagonBg()}`}></div>
                    <div class="position-absolute w-5 h-5 m-1 rounded-sm" style={`background-color:${themeVar.getMainHexagonIcon()}`}></div>
                    <div class="position-absolute w-5 h-5 m-1 rounded-sm" style={`background-color:${themeVar.getSubHexagonBg()}`}></div>
                    <div class="position-absolute w-5 h-5 m-1 rounded-sm" style={`background-color:${themeVar.getHoverHexagonBg()}`}></div></span>

                  <div class=" left-[30px] top-[-9px] hexPreview pointer-events-none hexTile absolute bg-transparent  cursor-pointer inline-block"
                    ...
                    <div style={`fill:${themeVar.getMainHexagonBg()}; transform-origin: center center; clip-path: polygon(0% 25%, 0% 75%, 50% 100%, 100% 75%, 100% 25%, 50% 0%); left: -2.5px; bottom: -2.5px; width: 88px; margin: 5px; height: 102.872px;`} class={` absolute cursor-pointer inline-block `} id="radiant:0">
                      <svg width="98" height="102.872">

                        <clipPath id="hexClip"><circle cx="40" cy="35" r="35"></circle><path id="radiant:0" d="
        ...
        Z
      " style="transform: translate(-0.1%, 5%);"></path></clipPath><path d="
        ...
        Z
      " style={`fill:${themeVar.getMainHexagonBorder()}; transform: translate(0.9%, 0.9%);`}></path><path d="
          ...
          {(themeVar: Themes, i) => (
            ...
            <Show when={themeVar.getThemeName() !== "Honey" && themeVar.getThemeName() !== "Light" && themeVar.getThemeName() !== "Dark"}>
              ...
              <div class=" mr-[11px] rounden-lg mt-10 focus:bg-accent hoverEffect card bg-neutral border">
                ...
                />   <label class="pointer-events-none" for="card1">
                  <FaSolidPen color={`${themeVar.getMainHexagonIcon()}`} size={21} class="z-20 absolute top-[40px] left-[72px]" />
                  <span class="colorPreview">
                    <div class="position-absolute w-5 h-5 m-1 rounded-sm" style={`background-color:${themeVar.getMainHexagonBg()}`}></div>
                    <div class="position-absolute w-5 h-5 m-1 rounded-sm" style={`background-color:${themeVar.getMainHexagonIcon()}`}></div>
                    <div class="position-absolute w-5 h-5 m-1 rounded-sm" style={`background-color:${themeVar.getSubHexagonBg()}`}></div>
